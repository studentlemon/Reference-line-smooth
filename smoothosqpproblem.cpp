...
void FemPosDeviationSqpOsqpInterface::CalculateKernel(
  ...
  if(num_of_points_< 2)
  {
      std::cout<<"num_of_points_< 2"<<std::endl;
  }
  ...
  if(col_num!= num_of_pos_variables_)
  {
      std::cout<<"col_num!= num_of_pos_variables_"<<std::endl;
  }
...
FemPosDeviationSqpOsqpInterface::CalculateLinearizedFemPosParams(
  ...
  {
      std::cout<<"index >= points.size() - 1"<<std::endl;
  }
...
void FemPosDeviationSqpOsqpInterface::CalculateAffineConstraint(
  ...
  const auto& ref_point_xy = ref_points_.front();
  (*upper_bounds)[0 * 2] = ref_point_xy.first + bounds_around_refs_[0];
  (*upper_bounds)[0 * 2 + 1] = ref_point_xy.second - 25.0;
  (*lower_bounds)[0 * 2] = ref_point_xy.first - bounds_around_refs_[0];
  (*lower_bounds)[0 * 2 + 1] = ref_point_xy.second - 26.0;
...
void FemPosDeviationSqpOsqpInterface::SetPrimalWarmStart(
  ...
  if(points.size()!= num_of_points_)
  {
      std::cout<<"points.size()!= num_of_points_"<<std::endl;
  }
